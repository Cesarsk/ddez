...
> **Status: real-org validated.** Twelve views (monitors, incidents, SLOs, logs,
> traces, services, events, RUM, synthetics, downtimes, costs, dashboards) plus `:overview`, a
...
| Key | Action |
|-----|--------|
| `:` | switch view: `:monitors` `:incidents` `:slos` `:logs` `:traces` `:services` `:events` `:rum` `:synthetics` `:downtimes` `:costs` `:dashboards` `:overview` `:ctx` `:settings` |
